app.controller("cellphoneCtrl", function ($scope) {
    ...
    $scope.add = function (model, color, price) {
        if (model && model.trim() !== '' &&
            color && color.trim() !== '' &&
            price && price !== '') {
            $scope.phones.push(new Phone(model, $scope.getColorNameByHex(color), price));
        }
    }
    ...
    $scope.save = function (id, model, color, price) {
        ...
        if (color && color.trim() !== '')
            $scope.phones[id].color = $scope.getColorNameByHex(color);
    ...
    $scope.turnEditModeOn = function (id) {
        ...
        $scope.editColor = $scope.getColorHexByName($scope.phones[id].color);
    ...
    $scope.getColorNameByHex = function (hex) {
        if (!hex) return;

        hex = hex.replace('#', '');
        let idx = colorHexes.indexOf(hex);
    ...
    $scope.getColorHexByName = function (name) {
        let idx = colorNames.indexOf(name.toLowerCase());
